...
public class ConversionController {
    ...
    @Autowired private SwopExchangeRateService swopExchangeRateService;

    @PostMapping("/")
    public BigDecimal convertCurrency(@RequestBody ConversionDto dto)
            throws IllegalArgumentException,
                    IllegalStateException,
                    IOException,
                    InterruptedException {
        logger.debug(
                "ConversionController: Request to convert {} {} to {}",
                dto.getAmount(),
        ...
        try {
            ...
            BigDecimal exchangeRate =
                    swopExchangeRateService.getExchangeRate(
                            dto.getSourceCurrency(), dto.getTargetCurrency());
            BigDecimal result =
                    ConversionUtil.convertToTargetCurrency(dto.getAmount(), exchangeRate);

            logger.debug(
                    "ConversionController: Conversion successful, result: {} {}",
                    result,
                    dto.getTargetCurrency());
